...
let state={view:'today',status:'all',subcategory:'all',month:new Date(),tasks:[],projects:[],events:[],courses:[],semesters:[],trash:{tasks:[],courses:[]},dailyPlan:{},dailyProjection:null,planningProfile:null,planBlocks:[],planState:null,planPreview:null,scheduleWeek:new Date(),semesterFilter:'',schedulePreview:null,loading:true,aiStatus:null,aiPreview:null,aiLoading:false,aiSettings:null,preferences:null,zoteroSources:[],researchInbox:[],researchProjectId:'',researchProjectItems:[],zoteroCollections:[],researchImportPreview:null,researchTaskPreview:null,appearance:structuredClone(APPEARANCE_DEFAULT),appearanceDraft:null,appearanceImageFile:null,appearancePreviewUrl:null,appearanceRemoveImage:false,focus:null,focusStats:null,focusSaving:false,focusImmersive:false,focusWarningPlayed:false};
...
async function loadTasks({migrate=true}={}){
  ...
  const [data,projectData,semesterData,courseData,eventData,planData,profileData,confirmedPlan,focusData,statsData,preferencesData,aiSettingsData,zoteroData,researchData]=await Promise.all([api('/api/tasks'),api('/api/projects'),api('/api/semesters'),api('/api/courses'),api(`/api/calendar/events?start=${rangeStart}&end=${rangeEnd}`),api(`/api/planning/daily?date=${today()}`),api('/api/planning/profile'),api(`/api/planning/plans?date=${today()}`),api('/api/focus/active'),api(`/api/focus/stats?start=${iso(addDays(new Date(),-6))}&end=${today()}`),api('/api/settings/preferences'),api('/api/settings/ai'),api('/api/research/sources'),api('/api/research/inbox')]);
  ...
  state.researchProjectItems=state.researchProjectId?(await api(`/api/research/projects/${state.researchProjectId}/items`)).items:[];
...
function updateChrome(){
  const titles={today:'今日',inbox:'收件箱',research:'科研工作台',course:'课程学习',personal:'个人生活',week:'未来 7 天',month:'月历',schedule:'课程表',research_library:'科研文献',trash:'回收站'};
...
function taskCard(task,dailyAllocation=null){
  ...
  const focus=activeFocusForTask(task),displayProgress=taskDisplayProgress(task);
  ...
  return `<article class="task-card ${task.domain} ${done?'done':''} ${focus?'focus-running':''} ${isOverdue(task)?'overdue':''}" data-task-context="${task.id}">
    ...
    <div class="task-main"><div class="task-title-row"><h3>${esc(task.title)}</h3><span class="status-badge ${task.status}">${STATUSES[task.status]}</span>${task.is_recurring?'<span class="badge">重复</span>':''}</div><p>${esc(meta)}</p>${focus?`<div class="task-focus-live" data-focus-task-live="${task.id}"><span><i></i>${focus.status==='paused'?'专注已暂停':'正在专注'}</span><time>${focusClock(focusElapsed(focus))}</time><b>${displayProgress}%</b></div>`:''}${task.description?`<p class="description">${esc(task.description)}</p>`:''}${task.tags?.length?`<div class="tag-row">${task.tags.map(tag=>`<span>#${esc(tag)}</span>`).join('')}</div>`:''}<div class="progress-track" data-task-progress="${task.id}" title="完成度 ${displayProgress}%"><i style="width:${displayProgress}%"></i></div></div>
...
function taskDisplayProgress(task){const focus=activeFocusForTask(task),estimated=Math.max(0,Number(task.estimated_minutes)||0),recorded=Math.max(0,Number(task.actual_minutes)||0),live=focus?focusElapsed(focus)/60:0,measured=estimated?Math.min(99,Math.floor((recorded+live)*100/estimated)):0;return Math.max(Number(task.progress)||0,measured)}
function renderActiveFocusBanner(){const focus=state.focus;if(!focus||focus.session_type!=='focus')return '';const task=focusTask();if(!task)return '';const elapsed=focusElapsed(focus),target=Number(focus.target_seconds||0),remaining=focus.mode==='free'?null:Math.max(0,target-elapsed);return `<section class="active-focus-banner" data-active-focus-banner><div class="active-focus-signal"><i></i><span>ACTIVE FOCUS</span></div><div><small>${focus.status==='paused'?'本轮已暂停':focus.status==='awaiting_action'?'等待确认':'任务正在推进'}</small><strong>${esc(task.title)}</strong><p>已专注 ${focusClock(elapsed)}${remaining===null?' · 自由计时':` · 剩余 ${focusClock(remaining)}`} · 预计完成度 <b>${taskDisplayProgress(task)}%</b></p></div><button class="secondary-btn" id="resume-active-focus">返回专注</button></section>`}
...
function renderWeek(){
  ...
  const allDay=days.map(day=>{const date=iso(day),daily=tasks.filter(task=>task.start_date===date||task.due_date===date).sort(sortTasks);return `<div class="week-all-day-cell">${daily.map(task=>`<button class="week-task-chip ${task.domain} ${task.status==='completed'?'done':''}" data-task-context="${task.id}" data-edit="${task.id}"><b>${task.due_date===date?'D':'S'}</b><span>${esc(task.title)}</span></button>`).join('')||'<span class="week-free">留白</span>'}</div>`}).join('');
  const lanes=days.map((day,index)=>{const date=iso(day),classes=events.filter(event=>event.date===date),blocks=(state.planBlocks||[]).filter(block=>block.block_date===date),now=new Date(),nowMinute=now.getHours()*60+now.getMinutes(),nowLine=index===0&&nowMinute>=rangeStart&&nowMinute<=rangeEnd?`<i class="week-now-line" style="top:${(nowMinute-rangeStart)/60*hourHeight}px"><b>现在</b></i>`:'';return `<div class="week-time-lane ${index===0?'today':''}" style="height:${laneHeight}px">${nowLine}${classes.map(event=>{const style=position(event.start_time,event.end_time);return style?`<button class="week-timed-event course" style="${style};--event-color:${esc(event.color||'#4f77bb')}" data-course-menu="${event.course_id}" data-meeting-id="${event.meeting_id}" data-event-date="${event.date}"><time>${esc(event.start_time)}–${esc(event.end_time)}</time><b>${esc(event.title)}</b><span>${esc(event.location||'地点待定')}</span></button>`:''}).join('')}${blocks.map(block=>{const style=position(block.start_time,block.end_time),task=state.tasks.find(item=>item.id===block.task_id),title=block.block_type==='focus'?(task?.title||block.task_title||'专注任务'):planTypeLabel(block.block_type);if(!style)return '';return block.block_type==='focus'?`<button class="week-timed-event plan focus" style="${style}" data-plan-focus="${block.task_id}" data-plan-block="${block.id||''}"><time>${esc(block.start_time)}–${esc(block.end_time)}</time><b>${esc(title)}</b></button>`:`<div class="week-timed-event plan rest" style="${style}"><time>${esc(block.start_time)}–${esc(block.end_time)}</time><b>${esc(title)}</b></div>`}).join('')}</div>`}).join('');
  return `<div class="week-legend"><span><i class="course"></i>课程占用</span><span><i class="focus"></i>专注计划</span><span><b>D</b> Deadline</span><span><b>S</b> 计划开始</span></div><div class="week-scroll"><div class="week-timeline"><div class="week-corner">全天事项</div>${headers}<div class="week-all-day-label">任务</div>${allDay}<div class="week-time-axis" style="height:${laneHeight}px">${labels.join('')}</div>${lanes}</div></div>`;
}
...
function renderResearchLibrary(){
  ...
  const projectPapers=state.researchProjectItems.length?`<div class="research-inbox project-papers">${state.researchProjectItems.map(item=>`<article><div class="research-paper"><small>${esc(item.item_type)} · ${esc(item.source_name)}</small><h3>${esc(item.title)}</h3><p>${esc(researchCreators(item))}${item.published_at?` · ${esc(item.published_at)}`:''}</p></div><div class="research-actions">${item.zotero_uri?`<a class="secondary-btn" href="${esc(item.zotero_uri)}">在 Zotero 打开</a>`:''}</div></article>`).join('')}</div>`:'<div class="empty"><strong>这个项目还没有论文</strong>可按 Zotero 文件夹整批导入，或按标题、作者、年份检索导入。</div>';
  const projectSection=`<section class="research-project-section"><div class="section-head"><div><h2>科研项目论文</h2><p>导入只建立 Yantu 项目关联，不会修改 Zotero</p></div><span>${state.researchProjectItems.length} 篇</span></div><div class="research-project-toolbar">${projects.length?`<label class="compact-field"><span>科研项目</span><select id="research-project-select">${projectOptions}</select></label>`:'<p>尚未创建科研项目</p>'}<button class="secondary-btn" id="create-research-project">新建科研项目</button><button class="primary-btn" id="open-research-import" ${!source||!projects.length?'disabled':''}>从 Zotero 导入论文</button></div>${projectPapers}</section>`;
  const list=items.length?`<div class="research-inbox">${items.map(item=>`<article><div class="research-paper"><small>${esc(item.item_type)} · ${esc(item.source_name)}</small><h3>${esc(item.title)}</h3><p>${esc(researchCreators(item))}${item.published_at?` · ${esc(item.published_at)}`:''}</p>${item.abstract?`<span>${esc(item.abstract)}</span>`:''}</div><div class="research-actions">${item.zotero_uri?`<a class="secondary-btn" href="${esc(item.zotero_uri)}">在 Zotero 打开</a>`:''}<button class="secondary-btn" data-research-dismiss="${item.id}">忽略</button><button class="primary-btn" data-research-task="${item.id}">创建阅读任务</button></div></article>`).join('')}</div>`:'<div class="empty"><strong>科研收件箱为空</strong>同步后，新收集的论文会先出现在这里，不会自动创建任务。</div>';
...
async function refreshResearch(){const [sources,inbox]=await Promise.all([api('/api/research/sources'),api('/api/research/inbox')]);state.zoteroSources=sources.sources;state.researchInbox=inbox.items}
async function refreshProjectPapers(projectId=state.researchProjectId){state.researchProjectId=projectId;state.researchProjectItems=projectId?(await api(`/api/research/projects/${projectId}/items`)).items:[]}
...
function updateFocusTaskIndicators(){const focus=state.focus;if(!focus||focus.session_type!=='focus')return;const task=focusTask();if(!task)return;const elapsed=focusElapsed(focus),progress=taskDisplayProgress(task),remaining=focus.mode==='free'?null:Math.max(0,Number(focus.target_seconds||0)-elapsed);$$(`[data-focus-task-live="${task.id}"]`).forEach(node=>{const label=node.querySelector('span'),time=node.querySelector('time'),value=node.querySelector('b');if(label)label.innerHTML=`<i></i>${focus.status==='paused'?'专注已暂停':focus.status==='awaiting_action'?'等待确认':'正在专注'}`;if(time)time.textContent=focusClock(elapsed);if(value)value.textContent=`${progress}%`});$$(`[data-task-progress="${task.id}"]`).forEach(node=>{node.title=`完成度 ${progress}%`;const bar=node.querySelector('i');if(bar)bar.style.width=`${progress}%`});const banner=$('[data-active-focus-banner]');if(banner){const paragraph=banner.querySelector('p');if(paragraph)paragraph.innerHTML=`已专注 ${focusClock(elapsed)}${remaining===null?' · 自由计时':` · 剩余 ${focusClock(remaining)}`} · 预计完成度 <b>${progress}%</b>`}}
...
async function startFocus(){const taskId=$('#focus-task').value,mode=$('#focus-mode').value,minutes=Math.max(1,Number($('#focus-custom-minutes').value)||25);if(!taskId)return;try{const data=await api('/api/focus/sessions',{method:'POST',body:JSON.stringify({task_id:taskId,plan_block_id:$('#focus-panel').dataset.planBlock||null,mode,target_seconds:mode==='free'?0:Math.round(minutes*60)})});state.focus=data.session;const task=state.tasks.find(item=>item.id===taskId);if(task&&['not_started','waiting'].includes(task.status))task.status='in_progress';state.focusWarningPlayed=false;focusTone('start');render();renderFocusPanel();updateFocusTaskIndicators();toast(mode==='free'?'自由专注已开始':`${minutes} 分钟番茄钟已开始`)}catch(error){showError(error)}}
