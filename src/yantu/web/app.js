...
const FOCUS_STORAGE='yantu.focus.timer.v1';
let state={view:'today',status:'all',subcategory:'all',month:new Date(),tasks:[],events:[],courses:[],semesters:[],trash:{tasks:[],courses:[]},dailyPlan:{},planningProfile:null,planBlocks:[],planPreview:null,scheduleWeek:new Date(),semesterFilter:'',schedulePreview:null,loading:true,aiStatus:null,aiPreview:null,aiLoading:false,appearance:structuredClone(APPEARANCE_DEFAULT),appearanceDraft:null,appearanceImageFile:null,appearancePreviewUrl:null,appearanceRemoveImage:false,focus:null,focusSaving:false};
...
async function loadTasks({migrate=true}={}){
  ...
  const [data,semesterData,courseData,eventData,planData,profileData,confirmedPlan]=await Promise.all([api('/api/tasks'),api('/api/semesters'),api('/api/courses'),api(`/api/calendar/events?start=${rangeStart}&end=${rangeEnd}`),api(`/api/planning/daily?date=${today()}`),api('/api/planning/profile'),api(`/api/planning/plans?date=${today()}`)]);
...
function bindDynamic(){
  ...
  $$('[data-plan-focus]').forEach(button=>button.onclick=()=>openFocus(button.dataset.planFocus));
...
function restoreFocus(){try{const saved=JSON.parse(localStorage.getItem(FOCUS_STORAGE)||'null');if(saved&&saved.version===1)state.focus=saved}catch{localStorage.removeItem(FOCUS_STORAGE)}}
function persistFocus(){if(state.focus)localStorage.setItem(FOCUS_STORAGE,JSON.stringify(state.focus));else localStorage.removeItem(FOCUS_STORAGE)}
function focusElapsed(timer=state.focus){if(!timer)return 0;const running=timer.running?Math.max(0,(Date.now()-timer.last_resumed_at)/1000):0;return Math.max(0,Number(timer.accumulated_seconds||0)+running)}
...
function openFocus(taskId=''){const panel=$('#focus-panel');panel.classList.remove('hidden');panel.dataset.preselected=taskId;renderFocusPanel()}
function closeFocus(){$('#focus-panel').classList.add('hidden')}
function clearFocus(){state.focus=null;persistFocus();document.title='研途 · 研究生个人工作台';renderFocusPanel()}
function renderFocusPanel(){
  ...
  if(!state.focus){const selected=panel.dataset.preselected||activeTasks[0]?.id||'',profile=state.planningProfile||{use_pomodoro:true,focus_minutes:25,short_break_minutes:5};panel.innerHTML=`<div class="focus-head"><div><i></i><small>FOCUS NODE // READY</small><strong>启动专注</strong></div><button id="focus-close" aria-label="关闭专注面板">×</button></div><div class="focus-setup"><label><span>关联任务</span><select id="focus-task">${activeTasks.map(task=>`<option value="${task.id}" ${task.id===selected?'selected':''}>${esc(task.title)}</option>`).join('')}</select></label><label><span>计时方式</span><select id="focus-mode"><option value="profile">规划偏好 · ${profile.use_pomodoro?`${profile.focus_minutes} / ${profile.short_break_minutes}`:'自由专注'}</option><option value="pomodoro25">番茄钟 · 25 / 5</option><option value="pomodoro50">长番茄 · 50 / 10</option><option value="free">自由专注 · 正向计时</option></select></label><p>番茄钟结束后自动记录实际投入；自由专注由你决定何时结束。</p><button class="focus-primary" id="focus-start" ${activeTasks.length?'':'disabled'}>${activeTasks.length?'进入专注':'暂无可专注任务'}</button></div>`;$('#focus-close').onclick=closeFocus;$('#focus-start').onclick=startFocus;return}
  const elapsed=focusElapsed(),isBreak=state.focus.phase==='break',remaining=state.focus.mode==='free'?elapsed:Math.max(0,state.focus.duration_seconds-elapsed),progress=state.focus.mode==='free'?0:Math.min(1,elapsed/state.focus.duration_seconds),task=focusTask();
  const label=isBreak?'RECOVERY // BREAK':state.focus.mode==='free'?'FLOW // OPEN TIMER':'FOCUS // DEEP WORK';
  document.title=`${focusClock(remaining)} · ${isBreak?'休息':task?.title||state.focus.task_title||'专注'}`;
  panel.innerHTML=`<div class="focus-head"><div><i class="live"></i><small>${label}</small><strong>${isBreak?'神经缓冲':esc(task?.title||state.focus.task_title||'专注任务')}</strong></div><button id="focus-close" aria-label="收起专注面板">×</button></div><div class="focus-active"><div class="focus-orbit" style="--focus-progress:${progress*360}deg"><div><time>${focusClock(remaining)}</time><span>${state.focus.running?(isBreak?'恢复中':'专注中'):'已暂停'}</span></div></div><p>${isBreak?'短暂离开屏幕，让注意力重新充能。':state.focus.mode==='free'?'不使用番茄节拍，结束时仍会记录实际投入。':`本轮完成后进入 ${state.focus.break_seconds/60} 分钟休息。`}</p><div class="focus-actions"><button id="focus-pause">${state.focus.running?'暂停':'继续'}</button><button class="focus-primary" id="focus-finish">${isBreak?'结束休息':'完成并记录'}</button><button class="focus-ghost" id="focus-discard">放弃</button></div></div>`;
  $('#focus-close').onclick=closeFocus;$('#focus-pause').onclick=toggleFocusPause;$('#focus-finish').onclick=finishFocus;$('#focus-discard').onclick=discardFocus;
}
function startFocus(){const taskId=$('#focus-task').value,preset=$('#focus-mode').value,task=state.tasks.find(item=>item.id===taskId),profile=state.planningProfile||{use_pomodoro:true,focus_minutes:25,short_break_minutes:5};if(!task)return;const profileFree=preset==='profile'&&!profile.use_pomodoro,free=preset==='free'||profileFree,duration=preset==='profile'?profile.focus_minutes:preset==='pomodoro50'?50:25,breakMinutes=preset==='profile'?profile.short_break_minutes:preset==='pomodoro50'?10:5;state.focus={version:1,task_id:taskId,task_title:task.title,mode:free?'free':'pomodoro',phase:'focus',duration_seconds:free?0:duration*60,break_seconds:breakMinutes*60,started_at:new Date().toISOString(),last_resumed_at:Date.now(),accumulated_seconds:0,running:true};persistFocus();renderFocusPanel();toast(free?'自由专注已开始':`${duration} 分钟番茄钟已开始`)}
function toggleFocusPause(){if(!state.focus)return;if(state.focus.running){state.focus.accumulated_seconds=focusElapsed();state.focus.running=false}else{state.focus.last_resumed_at=Date.now();state.focus.running=true}persistFocus();renderFocusPanel()}
async function recordFocus(timer,elapsed){const minutes=Math.max(1,Math.round(elapsed/60));await api('/api/time-entries',{method:'POST',body:JSON.stringify({task_id:timer.task_id,start_time:timer.started_at,end_time:new Date().toISOString(),duration:minutes,note:timer.mode==='pomodoro'?`番茄专注 · ${minutes} 分钟`:`自由专注 · ${minutes} 分钟`})});return minutes}
async function finishFocus(){if(!state.focus||state.focusSaving)return;if(state.focus.phase==='break'){clearFocus();toast('休息结束，注意力已重新上线');return}state.focusSaving=true;try{const minutes=await recordFocus(state.focus,focusElapsed());clearFocus();await loadTasks({migrate:false});toast(`已记录 ${minutes} 分钟专注`)}catch(error){showError(error)}finally{state.focusSaving=false}}
function discardFocus(){if(!state.focus||confirm('放弃本次计时且不记录投入？')){clearFocus();toast('本次计时已放弃')}}
async function tickFocus(){if(!state.focus||!state.focus.running||state.focusSaving)return;const elapsed=focusElapsed();if(state.focus.mode==='pomodoro'&&elapsed>=state.focus.duration_seconds){if(state.focus.phase==='break'){clearFocus();toast('休息结束，可以开始下一轮');return}state.focusSaving=true;const completed={...state.focus};try{const minutes=await recordFocus(completed,completed.duration_seconds);state.focus={version:1,task_id:completed.task_id,task_title:completed.task_title,mode:'pomodoro',phase:'break',duration_seconds:completed.break_seconds,break_seconds:completed.break_seconds,started_at:new Date().toISOString(),last_resumed_at:Date.now(),accumulated_seconds:0,running:true};persistFocus();await loadTasks({migrate:false});toast(`番茄完成 · 已记录 ${minutes} 分钟`)}catch(error){state.focus=completed;state.focus.running=false;persistFocus();showError(error)}finally{state.focusSaving=false}}renderFocusPanel()}
...
$('#appearance-btn').onclick=openAppearance;$('#close-appearance').onclick=cancelAppearance;$('#cancel-appearance').onclick=cancelAppearance;$('#appearance-form').onsubmit=saveAppearance;
